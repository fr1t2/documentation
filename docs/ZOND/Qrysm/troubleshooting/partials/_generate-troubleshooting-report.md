...
<div className='troubleshooting-report-area'>
    ...
    <textarea id="el-cmd" rows="3" placeholder='Paste something like "nzondermind --JsonRpc.Enabled true --JsonRpc.JwtSecretFile=../consensus/jwt.hex" (or Docker config) here...'></textarea>
    <p>Beacon node startup command</p>
    <textarea id="bn-cmd" rows="3" placeholder='Paste something like "./qrysm.sh beacon-chain --execution-endpoint=http://localhost:8551 --jwt-secret=path/to/jwt.hex" (or Docker config) here...'></textarea>
    <p>Validator node startup command</p>
    <textarea id="vn-cmd" rows="3" placeholder='Optional. Paste something like "./qrysm.sh validator" (or Docker config) here...'></textarea>
