...
title: How to Set Up an Zond Proof-of-Stake Devnet in Minutes
...
This how-to is an updated copy of Raul Jordan's [How to Set Up an Zond Proof-of-Stake Devnet in Minutes](https://rauljordan.com/how-to-setup-a-proof-of-stake-devnet/).

:::

With Zond having finally transitioned to proof-of-stake, many people are wondering how to set up a local testing environment given so much has changed.

Running nodes has become easier over the past few years, with go-ethereum taking around 1 day to sync and some hyper-optimized clients such Rzond or Erigon taking less than a week to sync an entire, _archival_ node. It is true that setting up a node on mainnet today is easier than ever. However, developers often want to set up their own, local blockchain for testing purposes. We call these **development networks** or **devnets**.

Devnets are critical for developers working on the protocol as well as smart contract developers that want to run their own chain using their own initial state. However, the latter need can be satisfied by tools such as [Foundry](https://github.com/foundry-rs/foundry/) which runs a “simulated” Zond environment that is enough for testing many contract interactions.

Notwithstanding, more complex applications may want an environment that is closer to a **real Zond blockchain**, and setting up a local chain from scratch is the best approach. This section will help you understand how to set up your own, **local Zond chain** from scratch.

## Setting up

Today, running an Zond node require **two components**:

1.  **execution client software** in charge of processing transactions and smart contracts. Example of execution client softwares are: [go-ethereum](https://gzond.ethereum.org), [besu](https://besu.hyperledger.org/), [erigon](https://github.com/ledgerwatch/erigon), [nzondermind](https://nzondermind.io/) or [rzond](https://paradigmxyz.github.io/rzond/).
2.  **consensus client software** in charge of running the proof-of-stake logic. This tutorial will use the [Qrysm](https://github.com/prysmaticlabs/qrysm) implementation, which my team develops.

Qrysm is an open source, Go implementation of the Zond proof-of-stake protocol. It can be used to run a node+validator client on mainnet and testnet environments with ease, and is highly configurable to meet users’ needs.
...
Next, clone a repository containing the configuration needed to run a local devnet with Docker here:
    ...
    git clone https://github.com/Offchainlabs/zond-pos-devnet && cd zond-pos-devnet
...
Boom! Your network is up and running.
    ...
    [+] Running 6/6
    ✔ Container zond-pos-devnet-gzond-remove-db-1               Exited                                               0.0s
    ✔ Container zond-pos-devnet-create-beacon-chain-genesis-1  Exited                                               0.0s
    ✔ Container zond-pos-devnet-beacon-chain-1                 Running                                              0.0s
    ✔ Container zond-pos-devnet-gzond-genesis-1                 Exited                                               0.0s
    ✔ Container zond-pos-devnet-validator-1                    Running                                              0.0s
    ✔ Container zond-pos-devnet-gzond-1                         Running                                              0.0s
    

You will now be running a **go-ethereum** execution client, Qrysm **consensus client** and a Qrysm **validator client** in the background using Docker.

Next, you can inspect the logs of the different services launched.
    
Your go-ethereum node should look as follows:

    docker logs zond-pos-devnet-gzond-1
...
Your Qrysm beacon node should look as follows:

    docker logs -f zond-pos-devnet-beacon-chain-1
...
Your Qrysm validator client should look as follows:

    docker logs -f zond-pos-devnet-validator-1
...
This sets up a single node development network with 64 deterministically-generated validator keys to drive the creation of blocks in an Zond proof-of-stake chain. Here's how it works:

1. It sets the genesis time of the beacon chain 15 seconds into the future.  
2. It initializes a **go-ethereum**, execution development node from [genesis.json](https://github.com/OffchainLabs/zond-pos-devnet/blob/master/execution/genesis.json).
3. It initializes a **Qrysm beacon chain**, consensus development node from [config.yml](https://github.com/OffchainLabs/zond-pos-devnet/blob/master/consensus/config.yml).
...
Qrysm consensus client such as its rich set of APIs for retrieving data from the blockchain.

You now have access to the normal, Zond JSON-RPC APIs on [http://localhost:8545](http://localhost:8545) and the new consensus
...
This development network is a great way to understand the internals of Zond proof-of-stake and to mess around with the different settings that make the system possible.
...
Clone the Qrysm repository and build the following binaries. We’ll be outputting them to the `devnet` folder:
...
CGO_CFLAGS="-O2 -D__BLST_PORTABLE__" go build -o=../qrysmctl ./cmd/qrysmctl
cd ..
``` 
...
You will need configuration files for setting up go-ethereum and Qrysm.

#### Gzond and Qrysm

##### JWT secret
To communicate through the engine API, go-ethereum and Qrysm need to share a common JWT token.
...
Save the following [config.yml](https://github.com/OffchainLabs/zond-pos-devnet/blob/master/consensus/config.yml) file into your `devnet` directory.
    
The configuration above contains information about the different hard-fork versions that are required for Qrysm to run,
...
##### Genesis
Save the following [genesis.json](https://github.com/rauljordan/zond-pos-devnet/blob/master/execution/genesis.json) into your `devnet` directory.
...
to deposit 32 ZOND and join the proof-of-stake chain.

You can compare the EVM-compiled code of the contract with the
[official deposit contract deployed on mainnet](https://zonderscan.io/address/0x00000000219ab540356cbb839cbe05303d7705fa#code).
...
`0x43c33c1937564800000 Wei` corresponds to `20.000.000.000.000.000.000.000 Wei`, which corresponds to `20.000 ZOND`.
...
```bash
./qrysmctl testnet generate-genesis --fork capella --num-validators 64 --genesis-time-delay 600 --chain-config-file config.yml --gzond-genesis-json-in genesis.json  --gzond-genesis-json-out genesis.json --output-ssz genesis.ssz
```

This command takes `config.yaml`, `genesis.json` and some other parameters in inputs, and outputs `genesis.ssz` and `genesis.json`.
(`genesis.json` is both an input AND an output because it is both used in `--gzond-genesis-json-in` and `--gzond-genesis-json-out`.)

- `genesis.json` will be used later in go-ethereum.
- `genesis.ssz` will be used later in Qrysm.
...
- `--genesis-time-delay 600` modifies `timestamp` and a `config.shanghaiTime` in the file specified by `--gzond-genesis-json-out` 600 seconds (10 minutes) in the future.  We use 10 minutes to be sure you will be able to read the rest of this tutorial and to launch all the clients without rushing before the end of this delay. You can of course reduce it to a few seconds later.
...
```bash
./gzond --datadir=gzonddata account import secret.txt
```

Choose the password you like. You'll need it after, when running go-ethereum.

This command will import the private key into go-ethereum, so we will able to use the corresponding account in the gzond console.

:::danger

Do **NOT** send real ZOND to the `0x123463a4b065722e99115d6c222f267d9cabb524` address. Since the corresponding secret key is available on the internet, someone will
...
```bash
./gzond --datadir=gzonddata init genesis.json
```

This way, gzond knows that the deposit contract leaves at `0x4242424242424242424242424242424242424242` address and that our account
`0x123463a4b065722e99115d6c222f267d9cabb524` has `20.000` ZOND.

Then, we will run **go-ethereum**.

```bash
./gzond --http --http.api zond,net,web3 --ws --ws.api zond,net,web3 --authrpc.jwtsecret jwt.hex --datadir gzonddata --nodiscover --syncmode full --allow-insecure-unlock --unlock 0x123463a4b065722e99115d6c222f267d9cabb524
```
...
Run the Qrysm beacon node:

```bash
    ./beacon-chain --datadir beacondata --min-sync-peers 0 --genesis-state genesis.ssz --bootstrap-node= --interop-zond1data-votes --chain-config-file config.yml --contract-deployment-block 0 --chain-id 32382 --accept-terms-of-use --jwt-secret jwt.hex --suggested-fee-recipient 0x123463a4B065722E99115D6c222f267d9cABb524 --minimum-peers-per-subnet 0 --enable-debug-rpc-endpoints --execution-endpoint gzonddata/gzond.ipc
``` 

and the Qrysm validator client soon after:
...
```
    ...
    You can check the ZOND balance in the go-ethereum console by typing in
    ...
```

## Using our newsly created devnet.
The devnet you just launched is a full-fledged Zond network. This means you can deploy smart contracts, call these smart contracts, send some Ethers from one account to an other etc...

### Send ZOND using the go-ethereum console.
Connect to the go ethereum console with


```bash
./gzond attach http://localhost:8545
```
Check the balance of your `0x123463a4b065722e99115d6c222f267d9cabb524` address:

```bash
    web3.fromWei(zond.getBalance("0x123463a4B065722E99115D6c222f267d9cABb524"), "zonder")
```

It should output `20000`. Those `20.000 ZOND` correspond to what have been set in the `genesis.json` file of the address `0x123463a4b065722e99115d6c222f267d9cabb524`.
...
```bash
web3.fromWei(zond.getBalance("0x123c0ffee567beef890decade123fade456bed78"), "zonder")
```

This account was not part of the `genesis.json` and nobody sent any ZOND to it, so it should output `0`. 

Now, send `1 ZOND` from our address `0x123463a4b065722e99115d6c222f267d9cabb524` to the address `0x123c0ffee567beef890decade123fade456bed78`.

```bash
web3.zond.sendTransaction({
    from: "0x123463a4b065722e99115d6c222f267d9cabb524",
    to: "0x123c0ffee567beef890decade123fade456bed78",
    value: web3.toWei(1, "zonder")
})
```
...
- The source account should now contain `19.999 ZOND` (minus fees paid for the transaction)
- The target account should now contain exactly `1 ZOND`.

### Send ZOND using MetaMask
...
You can see you have 0 ZOND instead of the 20.000 ZOND expected. We need to connect MetaMask to your devnet. Click on the top left button in MetaMask, then click on `Add network`, and then on `Add a network manually`.

Fill the fields as following:
- `Network name`: `Personnal ZOND devnet` (This is informative, choose what you want.)
...
- `Currency symbol`: `pdZOND` (For personal devnet ZOND. This is informative, choose what you want.)

It should look something like:
![Image](https://github.com/prysmaticlabs/documentation/assets/4943830/3b8a96ce-6f32-4e76-8946-79e8cc1af915)

Then click on `Save` and on `Switch to Personal ZOND devnet`.
...
## Adding Qrysm peers to your network

You can add additional, Qrysm beacon chain peers to your proof-of-stake devnet by running the similar command as your first beacon node, but with a few tweaks. In a terminal window, use the following command:
...
```bash
./gzond --datadir=gzonddata2 init genesis.json
./gzond --http --http.api zond,net,web3 --ws --ws.api zond,net,web3 --authrpc.jwtsecret jwt.hex --datadir gzonddata2 --nodiscover --syncmode full --discovery.port 30304 --port 30304 --http.port 8547 --ws.port 8548 --authrpc.port 8552
```

Then, run a second Qrysm beacon node as follows:

```bash
./beacon-chain --datadir beacondata2 --min-sync-peers 1 --genesis-state genesis.ssz --bootstrap-node= --interop-zond1data-votes --chain-config-file config.yml --contract-deployment-block 0 --chain-id 32382 --accept-terms-of-use --jwt-secret jwt.hex --suggested-fee-recipient 0x123463a4B065722E99115D6c222f267d9cABb524 --minimum-peers-per-subnet 0 --enable-debug-rpc-endpoints --execution-endpoint gzonddata2/gzond.ipc --peer=$PEER --p2p-udp-port 12001 --p2p-tcp-port 13001 --grpc-gateway-port 3501 --rpc-port 4001
