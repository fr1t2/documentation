...
To sync from a checkpoint, your Qrysm beacon node needs three pieces of information: the latest finalized `BeaconState`, the `SignedBeaconBlock`, and (if you're on a testnet) the genesis state for the network you're using. Togzonder, the `BeaconState` and `SignedBeaconBlock` represent a single **checkpoint state**.
...
Start your Qrysm beacon node with the `--checkpoint-sync-url` flag set to a fully synced beacon node's RPC gateway provider endpoint. This endpoint is usually exposed via port `3500`. Set the `--genesis-beacon-api-url` flag to the same URL if you want to fetch the genesis state along with the `BeaconState` and `SignedBeaconBlock`. You may also use a public beacon chain checkpoint sync endpoint at your own risk at https://zond-clients.github.io/checkpoint-sync-endpoints/ .
...
```
./qrysm.bat beacon-chain --checkpoint-sync-url=http://localhost:3500 --genesis-beacon-api-url=http://localhost:3500
```
...
```bash
./qrysm.sh beacon-chain --checkpoint-sync-url=http://localhost:3500 --genesis-beacon-api-url=http://localhost:3500
```
...
The above instructions tell you how to **request** checkpoint state from another node. If you want to **serve** these requests, run a fully synced node with the following flags:
 ...
 - `--enable-debug-rpc-endpoints`: The [Beacon Node API for retrieving a BeaconState](https://ethereum.github.io/beacon-APIs/#/Debug/getStateV2) is a debug endpoint - this flag tells Qrysm to enable the endpoint so checkpoint sync requests can be served through your beacon node's RPC gateway provider endpoint.
...
Issue the following commands to export the `BeaconState` and `SignedBeaconBlock` files from a synced beacon node using `qrysmctl`. Until `qrysmctl` is included in Qrysm's binary release package, it is necessary to run it from a local source checkout:

:::info

Installing `qrysmctl` via `qrysm.sh`, or downloading it from qrysm's github release page, will be possible in an upcoming stable release.

:::

```bash
$ git clone git@github.com:prysmaticlabs/qrysm.git
Cloning into 'qrysm'...
...
$ go run github.com/prysmaticlabs/qrysm/v3/cmd/qrysmctl checkpoint-sync download --beacon-node-host=http://localhost:3500
```

You should see the following output if your export was successful:

```bash
INFO[0000] requesting http://localhost:3500/zond/v2/debug/beacon/states/finalized
INFO[0001] detected supported config in remote finalized state, name=holesky, fork=bellatrix
INFO[0001] requesting http://localhost:3500/zond/v2/beacon/blocks/0x766bdce4c70b6ee991bd68f8065d73e3990895b1953f6b931baae0502d8cbfcf
...
```
curl -H "Accept: application/octet-stream"  http://localhost:3500/zond/v1/debug/beacon/states/genesis > genesis.ssz
```

You can also just manually download the genesis state from GitHub: [holesky](https://github.com/zond-clients/holesky/blob/main/custom_config_data/genesis.ssz) | [Sepolia](https://github.com/zond-clients/sepolia/blob/main/bepolia/genesis.ssz)
...
1. Navigate to `http://localhost:3500/zond/v1/beacon/headers/finalized` using your browser.
...
4. Ensure that the `state_root` reported by the blockchain explorer matches the `state_root` you pulled from your browser. If you don't see a match, feel free to reach out to us on [Discord](https://theqrl.org/discord) and we'll help you troubleshoot.
...
**Does Qrysm's implementation of checkpoint sync support backfilling?**
Qrysm's current implementation syncs forward-only. Backfilling will be supported in a future Qrysm release. Note that backfilling isn't required to run a validator - it's only required if you want to run an archive node, support other peers, or query chain history through your beacon node.
...
A [community maintained list](https://zond-clients.github.io/checkpoint-sync-endpoints/) of public Beacon Chain checkpoint sync endpoints is maintained.

Feel free to ask on our [Discord server](https://theqrl.org/discord) if you need help identifying a **Mainnet** checkpoint state provider.

**Do I need to provide a genesis state when using checkpoint sync on Mainnet?** <br/>
No. Mainnet's genesis state is embedded within Qrysm.
...
**Where can I learn more about checkpoint sync?** <br/>
 ...
 - [How to: Checkpoint Sync](https://notes.ethereum.org/@launchpad/checkpoint-sync) by members of the Zond Foundation
