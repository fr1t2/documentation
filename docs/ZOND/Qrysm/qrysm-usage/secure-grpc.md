...
```text
./qrysm.sh beacon-node --tls-cert=server.pem --tls-key=server.key
```

and to use secure gRPC with a validator:

```text
./qrysm.sh validator --tls-cert=server.pem
```
...
```text
./qrysm.sh validator --tls-cert=ca.cert
```
...
1. Use the certificates to launch the beacon node:
    ...
    ```text
    ./qrysm.sh beacon-node --tls-cert=beacon.pem --tls-key=beacon.key
    ```
