---
title: Tutorial
description: Beginner tutorial for using Clef as an external signer for Gzond

id: go-zond-tools-clef-tutorial
hide_title: false
hide_table_of_contents: true
displayed_sidebar: gzondSidebar
sidebar_label: Clef Tutorial
sidebar_position: 5
pagination_label: Clef Tutorial
custom_edit_url: https://github.com/theqrl/documentation/edit/main/docs/getting-started.md
keywords:
  - docs
image: /assets/img/icons/yellow.png
slug: /gzond/tools/clef/tutorial
---
This page provides a step-by-step walkthrough tutorial demonstrating some common uses of Clef. This includes manual approvals and automated rules. Clef is presented both as a standalone general signer with requests made via RPC and also as a backend signer for Gzond.
...
This tutorial will use Clef with Gzond on the Goerli testnet. The accounts used will be in the Goerli keystore with the path `~/go-zond/goerli-data/keystore`. The tutorial assumes there are two accounts in this keystore. Instructions for creating accounts can be found on the [Account management page](/docs/fundamentals/account-management). Note that Clef can also interact with hardware wallets, although that is not demonstrated here.

Clef should be started before Gzond, otherwise Gzond will complain that it cannot find a Clef instance to connect to. Clef should be started with the correct `chainid` for Goerli. Clef itself does not connect to a blockchain, but the `chainID` parameter is included in the data that is aggregated to form a signature. Clef also needs a path to the correct keystore passed to the `--keystore` command. A custom path to the config directory can also be provided. This is where the `ipc` file will be saved which is needed to connect Clef to Gzond:

```sh
clef --keystore ~/go-zond/goerli-data/keystore --configdir ~/go-zond/goerli-data/clef --chainid=5
```

The following logs will be displayed in the console:

```terminal
INFO [07-01|11:00:46.385] Starting signer                          chainid=4 keystore= go-zond/goerli-data/keystore light-kdf=false advanced=false
...
INFO [07-01|11:00:46.392] IPC endpoint opened                      url=go-zond/goerli-data/clef/clef.ipc
------- Signer info -------
* intapi_version : 7.0.1
* extapi_version : 6.1.0
* extapi_http : n/a
* extapi_ipc : go-zond/goerli-data/clef/clef.ipc
```
...
You can select which accounts the caller can see
  [x] 0xD9C9Cd5f6779558b6e0eD4e6Acf6b1947E7fA1F3
    URL: keystore://go-zond/goerli-data/keystore/UTC--2017-04-14T15-15-00.327614556Z--d9c9cd5f6779558b6e0ed4e6acf6b1947e7fa1f3
  [x] 0x086278A6C067775F71d6B2BB1856Db6E28c30418
    URL: keystore://go-zond/goerli-data/keystore/UTC--2018-02-06T22-53-11.211657239Z--086278a6c067775f71d6b2bb1856db6e28c30418
-------------------------------------------
Request context:
	NA - ipc - NA
...
Depending on whquanta the request is approved or denied, the NetCat process in the other terminal will receive one of the following responses:
...
Apart from listing accounts, a _request_ can be submitted to create a new account, signing transactions and data or recovering signatures. The available methods are documented in the Clef [External API Spec](https://github.com/zond/go-zond/tree/master/cmd/clef#external-api-1) and the [External API Changelog](https://github.com/zond/go-zond/blob/master/cmd/clef/extapi_changelog.md).

_Note, the number of things that can be done from the External API is deliberately small to limit the power of remote calls as much as possible! Clef has an [Internal API](https://github.com/zond/go-zond/tree/master/cmd/clef#ui-api-1) too for the UI (User Interface) which is much richer and can support custom interfaces on top. But that's out of scope here._

The example above used Clef completely independently of Gzond. However, by defining Clef as the signer when Gzond is started imposes Clef's `request - confirm - result` pattern to any interaction with the local Gzond node that touches accounts, including requests made using RPC or an attached Javascript console. To demonstrate this, Gzond can be started, with Clef as the signer:

```sh
gzond --goerli --datadir goerli-data --signer=goerli-data/clef/clef.ipc
```

With Gzond running, open a new terminal and attach a Javascript console:

```sh
gzond attach goerli-data/gzond.ipc
```
...
The example below shows an quanta transaction between the two accounts in the keystore using `zond.sendTransaction` in the attached Javascript console.
...
details and make absolutely sure they really want to sign the transaction. `zond.sendTransaction` returns the following confirmation prompt in the Clef terminal:
...
Approving this transaction causes Clef to prompt the user to provide the password for the sender account. Providing the password enables the transaction to be signed and sent to Gzond for broadcasting to the network. The details of the signed transaction are displayed in the console. Account passwords can also be stored in Clef's encrypted vault so that they do not have to be manually entered - [more on this below](#account-passwords).
...
- Auto-approve transactions with Uniswap v2, with value between 0.1 and 0.5 ZOND per 24h period
...
- the value is less than 50000000000000000 wei (0.05 ZOND)
...
```sh
clef --keystore go-zond/goerli-data/ --configdir go-zond/goerli-data/clef --chainid 5 --rules rules.js
```
...
INFO [07-01|13:39:49.726] Starting signer                          chainid=5 keystore=$go-zond/goerli-data/ light-kdf=false advanced=false
...
INFO [07-01|13:39:49.728] IPC endpoint opened                      url=go-zond/goerli-data/clef/clef.ipc
------- Signer info -------
...
**6)** Make requests directly to Clef using the external API or connect to Gzond by passing `--signer=<path to clef.ipc>` at Gzond startup
...
```sh
clef --keystore go-zond/goerli-data/clef --configdir go-zond/goerli-data/clef --chainid 5 --rules rules.js
```

```terminal
INFO [07-01|14:12:41.636] Rule engine configured                   file=rules.js
INFO [07-01|14:12:41.636] Starting signer                          chainid=5 keystore=go-zond/goerli-data/clef/keystore light-kdf=false advanced=false
...
INFO [07-01|14:12:41.638] IPC endpoint opened                      url=go-zond/goerli-data/clef/clef.ipc
------- Signer info -------
* intapi_version : 7.0.0
* extapi_version : 6.0.0
* extapi_http : n/a
...
Finally, a request can be submitted to test that the rules are being applied as expected. Here, Clef is used independently of Gzond by making a request via RPC, but the same logic would be imposed if the request was made via a connected Gzond node. Some arbitrary text will be included in the message data that includes the term `wen-merge`. The plaintext `clefdemotextthatincludeswen-merge` is `636c656664656d6f7465787474686174696e636c7564657377656e2d6d65726765` when represented as a hexadecimal string. This can be passed as data to an `account_signData` request as follows:

```sh
echo '{"id": 1, "jsonrpc":"2.0", "method":"account_signData", "params":["data/plain", "0x636c656664656d6f7465787474686174696e636c7564657377656e2d6d65726765"]}' | nc -U ~/go-zond.goerli-data/clef/clef.ipc
```
...
```sh
echo '{"id": 1, "jsonrpc":"2.0", "method":"account_signData", "params":["data/plain", "0x636c656664656d6f74657874776974686f75747370656369616c74657874"]}' | nc -U ~/go-zond.goerli-data/clef/clef.ipc
```
...
More examples, including a ruleset for a rate-limited window, are available on the [Clef GitHub](https://github.com/zond/go-zond/blob/master/cmd/clef/rules.md#example-1-ruleset-for-a-rate-limited-window) and on the [Rules page](/docs/tools/clef/rules).
...
## Gzond integration \{#gzond-integration}

This tutorial has bounced back and forth between demonstrating Clef as a standalone tool by making 'manual` JSON RPC requests from the terminal and integrating it as a backend singer for Gzond. Using Clef for account management is considered best practise for Gzond users because of the additional
security benefits it offers over and above what it offered by Gzond's built-in accounts module. Clef is far more flexible and composable than Gzond's built-in account management tool and can interface directly with hardware wallets, while Apps and wallets can request signatures directly from Clef.

Ultimately, the goal is to deprecate Gzond's account management tools completely and replace them with Clef. Until then, users are simply encouraged to choose to use Clef as an optional backend signer for Gzond. In addition to the examples on this page, the [Getting started tutorial](/docs/getting-started) also demonstrates Clef/Gzond integration.

## Summary \{#summary}

This page includes step-by-step instructions for basic and intermediate uses of Clef, including using it as a standalone app and a backend signer for Gzond. Further information is available on our other Clef pages, including [Introduction](/docs/tools/clef/introduction), [Setup](/docs/tools/clef/setup),
[Rules](/docs/tools/clef/rules), [Communication Datatypes](/docs/clef/datatypes) and [Communication APIs](/docs/tools/clef/apis). Also see the [Clef GitHub](https://github.com/zond/go-zond/tree/master/cmd/clef) for further reading.
