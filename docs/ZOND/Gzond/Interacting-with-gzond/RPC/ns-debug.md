...
id: go-zond-interacting-rpc-ns-debug
hide_title: false
hide_table_of_contents: true
displayed_sidebar: gzondSidebar
sidebar_label: Debug Namespace
sidebar_position: 7
pagination_label: Debug Namespace
custom_edit_url: https://github.com/theqrl/documentation/edit/main/docs/getting-started.md
keywords:
  - docs
image: /assets/img/icons/yellow.png
slug: /gzond/interacting/rpc/ns-debug
---
...
If `incompletes` is false, then accounts for which the key preimage (i.e: the `address`) doesn't exist in db are skipped. NB: gzond by default does not store preimages.
...
References: [RLP](https://zond.org/en/developers/docs/data-structures-and-encoding/rlp/)
...
Gzond only keeps recent trie nodes and preimages of keys in memory - for older blocks this information is deleted by Gzond's garbage collection. This means that calls to `debug_GetModifiedAccountsByNumber` on blocks that are old enough to be eligible for garbage collection will return an error due to the trie nodes and preimages being unavailable. To fix this, run Gzond with `--cache.preimages=true` to prevent the relevant data being lost to the garbage collector 
...
References:
[Ethash](https://zond.org/en/developers/docs/consensus-mechanisms/pow/mining-algorithms/ethash/)
...
When JS-based tracing (see below) was first implemented, the intended usecase was to enable long-running tracers that could stream results back via a subscription channel. This method works a bit differently. (For full details, see [PR](https://github.com/zond/go-zond/pull/17914))
...
Files are created in a temp-location, with the naming standard `block_<blockhash:4>-<txindex>-<txhash:4>-<random suffix>`. Each opcode immediately streams to file, with no in-gzond buffering aside from whatever buffering the os normally does.
...
References:
[RLP](https://zond.org/en/developers/docs/data-structures-and-encoding/rlp/)
...
The `debug_traceCall` method lets you run an `zond_call` within the context of the given block execution using the final state of parent block as the base. The first argument (just as in `zond_call`) is a [transaction object](/docs/interacting-with-gzond/rpc/objects#transaction-call-object). The block can be specified either by hash or by number as the second argument. The trace can be configured similar to `debug_traceTransaction`, see [TraceCallConfig](#tracecallconfig). The method returns the same output as `debug_traceTransaction`.
...
please refer to the [subscription page](/docs/interacting-with-gzond/rpc/pubsub) for more details.
...
You can compose these basic patterns. If you want to see all output from peer.go in a package below zond (zond/peer.go, zond/downloader/peer.go) as well as output from package p2p at level \<= 5, use:

```js
debug.vmodule('zond/*/peer.go=6,p2p=5');
```
