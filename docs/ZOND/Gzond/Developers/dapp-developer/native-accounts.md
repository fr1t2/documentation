...
id: go-zond-daap-dev-native-accounts
hide_title: false
hide_table_of_contents: true
displayed_sidebar: gzondSidebar
sidebar_label: Account Management
sidebar_position: 3
pagination_label: Account Management
custom_edit_url: https://github.com/theqrl/documentation/edit/main/docs/getting-started.md
keywords:
  - docs
image: /assets/img/icons/yellow.png
slug: /gzond/dev/daap-dev/native-accounts
---

Gzond provides a simple, yet thorough accounts package that includes all the tools developers need to leverage all the security of Gzond's crypto implementation in a Go native application. The account management is done client side with all sensitive data held inside the application. This gives the user control over access permissions without relying on any third party.

**Note: Gzond's built-in account management is convenient and straightforward to use, but best practice is to use the external tool _Clef_ for key management.**

## Encrypted keystores \{#encrypted-keystores}

Access keys to Zond accounts should never be stored in plain-text. Instead, they should be stored encrypted so that even if the mobile device is accessed by a malicious third party the keys are still hidden under an additional layer of security. Gzond provides a keystore that enables developers to store keys securely. The Gzond keystore uses [Scrypt](https://pkg.go.dev/golang.org/x/crypto/scrypt) to store keys that are encoded using the [`secp256k1`](https://www.secg.org/sec2-v2.pdf) elliptic curve. Accounts are stored on disk in the [Web3 Secret Storage](https://github.com/zond/wiki/wiki/Web3-Secret-Storage-Definition) format. Developers should be aware of these implementation details
...
The encrypted keystore is implemented by the [`accounts.Manager`](https://godoc.org/github.com/zond/go-zond/accounts#Manager) struct from the [`accounts`](https://godoc.org/github.com/zond/go-zond/accounts) package, which also contains the configuration constants for the _standard_ or _light_ security modes described above. Hence client side account management
simply requires importing the `accounts` package into the application code.

```go
import "github.com/zond/go-zond/accounts"
import "github.com/zond/go-zond/accounts/keystore"
import "github.com/zond/go-zond/common"
```
...
An Zond account is implemented by the [`accounts.Account`](https://godoc.org/github.com/zond/go-zond/accounts#Account) struct from the Gzond [accounts](https://godoc.org/github.com/zond/go-zond/accounts) package. Assuming an instance of an [`accounts.Manager`](https://godoc.org/github.com/zond/go-zond/accounts#Manager) called `am` exists, all of the described lifecycle operations can be executed with a handful of function calls (error handling omitted).
...
_Although instances of [`accounts.Account`](https://godoc.org/github.com/zond/go-zond/accounts#Account) can be used to access various information about specific Zond accounts, they do not contain any sensitive data (such as passphrases or private keys), rather they act solely as identifiers for client code and the keystore._

## Signing authorization \{#signing-authorization}

Account objects do not hold the sensitive private keys of the associated Zond accounts. Account objects are placeholders that identify the cryptographic keys. All operations that require authorization (e.g. transaction signing) are performed by the account manager after granting it access to the private keys.
...
Assuming an instance of an [`accounts.Manager`](https://godoc.org/github.com/zond/go-zond/accounts#Manager) called `am` exists, a new account can be created to sign transactions using [`NewAccount`](https://godoc.org/github.com/zond/go-zond/accounts#Manager.NewAccount). Creating transactions is out of scope for this page so instead a random [`common.Hash`](https://godoc.org/github.com/zond/go-zond/common#Hash) will be signed instead.
...
Note that [`SignWithPassphrase`](https://godoc.org/github.com/zond/go-zond/accounts#Manager.SignWithPassphrase) takes an [`accounts.Account`](https://godoc.org/github.com/zond/go-zond/accounts#Account) as the signer, whereas [`Sign`](https://godoc.org/github.com/zond/go-zond/accounts#Manager.Sign) takes only a [`common.Address`](https://godoc.org/github.com/zond/go-zond/common#Address). The reason for this is that an [`accounts.Account`](https://godoc.org/github.com/zond/go-zond/accounts#Account) object may also contain a custom key-path, allowing [`SignWithPassphrase`](https://godoc.org/github.com/zond/go-zond/accounts#Manager.SignWithPassphrase) to sign using accounts outside of the keystore; however [`Sign`](https://godoc.org/github.com/zond/go-zond/accounts#Manager.Sign) relies on accounts already unlocked within the keystore, so it cannot specify custom paths.

## Summary \{#summary}

Account management is a fundamental pillar of Zond development. Gzond's Go API provides the tools required to integrate best-practice account security into Go native applications using a simple set of Go functions.
