...
id: go-zond-evm-tracing-live-tracing
hide_title: false
hide_table_of_contents: true
displayed_sidebar: gzondSidebar
sidebar_label: Live Tracing
sidebar_position: 4
pagination_label: Live Tracing
custom_edit_url: https://github.com/theqrl/documentation/edit/main/docs/getting-started.md
keywords:
  - docs
image: /assets/img/icons/yellow.png
slug: /gzond/dev/evm-tracing/live-tracing
---

Gzond v1.14.0 introduces a new optional feature, allowing users to stream (a subset of) all observable blockchain data in real-time. By writing some Go code you can develop a data indexing solution which will receive events from Gzond as it is syncing and processing blocks. You may find the full list of events in the source code [here](https://github.com/zond/go-zond/blob/master/core/tracing/hooks.go), but below is a summary:

- Initialization: receives chain configuration including hard forks and chain ID. Also receives the genesis block.
- Block processing: receives the block which gzond will process next and any error encountered during processing.
- Transaction processing: receives the transaction which gzond will process next and the receipt post-execution.
- EVM:
...
Gzond ships with a built-in live tracer which tracks the quanta supply changes of blocks. This would be of interest to users wishing to e.g. compute the total supply of ether.

### Running

If the goal is to compute the total Quanta supply, a full sync must be performed. The supply tracer must be enabled in the configuration of the node. This can be done as such:

```terminal
./build/bin/gzond --syncmode full --vmtrace supply --vmtrace.jsonconfig '{"path": "output"}'
```

Gzond will start full-syncing and right away the tracer will emit supply changes of each block to files inside the given directory. The path to this directory must be configured via the `--vmtrace.jsonconfig` flag as demonstrated above.
...
| issuance     | Object      | Contributors to newly issued Quanta |
| burn         | Object      | Contributors to burn Quanta         |

`issuance` itself is comprised of:

| field         | type        | description                                     |
| ------------- | ----------- | ----------------------------------------------- |
| genesisAlloc  | string      | Quanta allocations of the genesis block in hex   |
| reward        | string      | block reward in hex (not relevant post-merge)   |
| withdrawals   | string      | Quanta withdrawn from a validators stake in hex  |

And `burn` is comprised of:

| field         | type        | description                                          |
| ------------- | ----------- | ---------------------------------------------------- |
| 1559          | string      | Quanta burnt as per EIP-1559 rules in hex             |
| blob          | string      | Quanta burnt as per EIP-4788 rules in hex             |
| misc          | string      | Quanta burnt due to selfdestruct edge-cases in hex    |
...
- Custom native tracers are invoked through the API and will be instantiated for each request. Live tracers are instantiated once on startup and used throughout the lifetime of Gzond.
...
Below is a tracer that tracks changes of Quanta supply across blocks.

### Set-up

First follow the instructions to [clone and build](/docs/getting-started/installing-gzond) Gzond from source code.

### Tracer code

Save the following snippet to a file under `zond/tracers/live/`.
...
import (
	...
	"github.com/zond/go-zond/consensus/misc/eip4844"
...
- Tracers don't have access to Gzond's database. They will have to implement their own persistence layer or a way to extract data. In this example, the tracer logs the data to a file.
- Note that we are resetting the delta on every new block, because the same tracer instance will be used all the time.

### Running the tracer

First compile the source by running `make gzond`. Then run the following command:

```bash
./build/bin/gzond --vmtrace supply --vmtrace.jsonconfig '{"config": "supply-logs"}' [OTHER_GZOND_FLAGS]
```
