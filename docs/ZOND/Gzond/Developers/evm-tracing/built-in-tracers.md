...
description: Explanation of the tracers that come bundled in Gzond as part of the tracing API.

id: go-zond-evm-tracing-built-in-tracing
hide_title: false
hide_table_of_contents: true
displayed_sidebar: gzondSidebar
sidebar_label: Built-In Tracing
sidebar_position: 3
pagination_label: Built-In Tracing
custom_edit_url: https://github.com/theqrl/documentation/edit/main/docs/getting-started.md
keywords:
  - docs
image: /assets/img/icons/yellow.png
slug: /gzond/dev/evm-tracing/built-in-tracing
---


Gzond comes bundled with a choice of tracers that can be invoked via the [tracing API](/docs/interacting-with-gzond/rpc/ns-debug). Some of these built-in tracers are implemented natively in Go, and others in Javascript. The default tracer is the opcode logger (otherwise known as struct logger) which is the default tracer for all the methods. Other tracers have to be specified by passing their name to the `tracer` parameter in the API call.
...
There are also a set of tracers written in Javascript. These are less performant than the Go native tracers because of overheads associated with interpreting the Javascript in Gzond's Go environment.
...
It is possible to give temporary state modifications to Gzond in order to simulate the effects of `zond_call` or `debug_traceCall`. For example, some new bytecode could be deployed to some address _temporarily just for the duration of the execution_ and then a transaction interacting with that address can be traced. This can be used for scenario testing or determining the outcome of some hypothetical transaction before executing for real.
...
Similar to [State overrides](#state-overrides), it is also possible to override some of the execution's block context to simulate the effects of `zond_call` or `debug_traceCall`, we support the following override options:
