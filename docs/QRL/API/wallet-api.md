...
:::note
Ensure the tree height is large enough for your needs and transfer all funds out of the address before all [OTS keys](./qrl/build/fundamentals/ots-keys) are used!
:::
...
For more information on the QRL Message Encoding please see the [Message documentation](../../qrl/use/tools/messages/overview)
:::
...
Additionally you can use another API to lookup the address details and tokens held by address. For instance the [GetTokensByAddresss Method](./qrl/api/qrl-public-api#gettokensbyaddress) will return a list of tokens held by that address.
:::
...
| Field | Type | Required| Details | 
| :--: | :---: | :--: | :--- |
| `slave_pks` | repeated bytes | YES | Array of slave PK's generated using a function similar to the [QRL Node CLI `slave_tx_generate`](../../qrl/use/node/node-cli/overview#slave_tx_generate) command. |
